...
GUI.sign = async function (sdsTool, keyId, data, digestMethod, rngSeedMaterial, signWithKeyId, signAsPlainText) {
    ...
    const key = sdsTool.keyDatabase.getKeyById(keyId)
    ...
    try {
        if(jsr !== undefined && !signAsPlainText) {
            ...
            const msg = alias => jsr.requestSignaturesFrom === undefined ?
                "Signed JSON sign request with key '" + alias + "'" : "Signed JSON sign request"
